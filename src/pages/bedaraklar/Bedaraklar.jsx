...
export default function Bedaraklar() {
  ...
  const postDataBedarak = async () => {
    try {
      ...
      formData.append("cathWantedImages", PolisFotosBedarak);
      formData.append("wantedFingerPrint", PolisfingerBedarak);

      formData.append("comment", HodimHulosasiBedaraklar.current.value);

      const res = await fetch("http://localhost:8081/api/v1/addCathWanted", {
        method: "POST",
        headers: {
